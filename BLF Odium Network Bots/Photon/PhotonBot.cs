...
namespace OdiumPhoton.Core
{
    ...
    public class PhotonBot : LoadBalancingClient
    {
        ...
        private void UpdateLoop()
        {
            while (true)
            {
                if (_isActive && _hasSpawned && _viewID > 0)
                {
                    BroadcastMovement();
                }
        ...
        {
            string displayName = $"[BOT] {BotName}";
            ...
            LocalPlayer.SetCustomProperties(new Hashtable
            {
                ...
                ["teamNumber"] = (byte)2,
                ["rank"] = (byte)new Random().Next(0, 255),
                ["killstreak"] = (byte)0
            });
        }
        ...
        public void SendChat(string message)
        {
            ...
            RPC(50, $"[BOT] {BotName}", message, (byte)255, (byte)255, (byte)255);
        }
        ...
        public void SendAnnouncement(string text, float duration = 5f)
        {
            ...
            {
                [(byte)0] = _viewID,
                [(byte)4] = new object[] { text, duration },
                [(byte)5] = (byte)61
            };

            OpRaiseEvent(RPC_EVENT, data,
                new RaiseEventOptions { CachingOption = EventCaching.AddToRoomCacheGlobal },
                SendOptions.SendReliable);
        }

        private void RPC(byte methodId, params object[] parameters)
        {
            var data = new Hashtable
            {
                [(byte)0] = _viewID,
        ...
        private string GenerateBotName() => $"{new Random().Next(1000, 10000)}";
